...
The table output shows that for every additional standard deviation of marriage rate (3.8), there is an increase of 0.6 divorces. Looking at the plots we can see that this relationship isn't as strong as the previous one. 
...
If we want a model that predicts divorce rate using bothe marriage rate and age at marriage, the methods are very similar to how we defined polynomial models before:
...
Interpretation: *Once we know median age at marriage for a State, there is listtle or no additional predictive power in also knowing the rate of marriage in that State.*
